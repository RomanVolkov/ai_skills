...
Locate and run the skill's `scripts/collect-site-metrics.py` (see Supporting Scripts section for how to find `$SKILL_DIR`) to gather:
...
The skill includes Python scripts in its `scripts/` subdirectory. Before running them, locate the skill directory:

```bash
SKILL_DIR=$(find ~/.claude/skills ~/.config/opencode/skills -maxdepth 1 -name "readme-generator" -type d -o -maxdepth 1 -name "readme-generator" -type l 2>/dev/null | head -1)
```
...
Usage:
```bash
python "$SKILL_DIR/scripts/collect-site-metrics.py" /path/to/repo
```
...
Usage:
```bash
python "$SKILL_DIR/scripts/validate-readme.py" README.md
```
